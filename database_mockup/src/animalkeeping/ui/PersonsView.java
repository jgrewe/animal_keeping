...
public class PersonsView  extends VBox implements Initializable, View {
    ...
    private void deletePerson() {
        if (selectedPerson.getTreatments().size() != 0) {
            Dialogs.showInfo("Cannot delete Person " + selectedPerson.getLastName() + " since it is referenced by treatment entries!");
            return;
        }
        ...
        session.close();
        personsTable.getSelectionModel().select(null);
        selectedPerson = null;
        personsTable.refresh();
    }
